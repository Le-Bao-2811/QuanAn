$(document).ready(function () {
	...
	var dTable = $("#tbDanhMuc").DataTable({
		processing: true,
		serverSide: true,
		ajax: "/admin/danhmucmenu/list",
		columns: [
			{ data: "id" },
			{ data: "danhMuc" },
	...
	$(document).on("click", ".js-add-or-update", function (ev) {
		...
		if ($(ev.currentTarget.parentElement.parentElement).attr('update') == '') {
			...
			//get data update
			$.get("/admin/danhmucmenu/get", { id: id },
				function (data, textStatus, jqXHR) {
					form.find("#Id").val(data.id);
					form.find("#DanhMuc").val(data.danhMuc);
				},
				"json"
			)
		}
		...
		modal.setPrimaryButtonEvent(function (ev) {
			...
			console.log(data);
			$.post("/admin/danhmucmenu/_addorUpdate", data,
				function (data, textStatus, jqXHR) {
					if (data == true) {
						dTable.draw(false);
						modal.hide();
